...
function my_login_logo() { ?>
  <style type="text/css">
      body.login div#login h1 a {
          	background-image: url(<?php echo get_bloginfo( 'template_directory' ) ?>/assets/images/logo-multilight.png);
            background-size: 90%;
			width: auto;
			height: 75px;
...
add_shortcode( 'quote', 'blog_quote_shortcode' );
function blog_quote_shortcode( $atts, $content = null ) {
   return '<div class="quote"><span>' . do_shortcode($content) . '</span></div>';
}

// formatting of images
add_shortcode( 'full_image_left', 'blog_full_image_left_shortcode' );
function blog_full_image_left_shortcode( $atts, $content = null ) {
   return '<div class="full-image left">' . do_shortcode($content) . '</div>';
}

add_shortcode( 'full_image_right', 'blog_full_image_right_shortcode' );
function blog_full_image_right_shortcode( $atts, $content = null ) {
   return '<div class="full-image right">' . do_shortcode($content) . '</div>';
}

add_shortcode( 'inline_images', 'blog_inline_images_shortcode' );
function blog_inline_images_shortcode( $atts, $content = null ) {
   return '<div class="inline-images">' . do_shortcode($content) . ' </div>';
}
